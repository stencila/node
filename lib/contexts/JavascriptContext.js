...
class JavascriptContext extends Context {
  ...
  async compileFunc (func) {
    ...
    for (let doc of docs) {
      ...
      let examples = []
      if (doc) {
        // Source is the documentation string
        source = doc
        // Strip spaces and asterisks from front of each line
        let jsdoc = source.replace(/^\s*\*?/mg, '')
        ...
        // Source is the function declaration
        source = code
      ...
      if (title) func.title = title
      if (summary) func.summary = summary
      if (descr) func.description = descr
    ...
    return funcs.length === 1 ? funcs[0] : funcs
  }
