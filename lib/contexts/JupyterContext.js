...
class JupyterContext extends Context {
  ...
  executeEval (eva) {
    ...
    return this._request('execute_request', content, 'execute_reply').then(({request, response}) => {
      const result = response.content.user_expressions.eval
  ...
  executeRun (run) {
    ...
    return this._request('execute_request', content, 'execute_result').then(({request, response}) => {
   ...
   */
  _request (requestType, content, responseType) {
    return new Promise((resolve, reject) => {
      ...
      if (this.timeout >= 0) {
        setTimeout(() => {
          if (this._requests[request.header.msg_id]) {
            reject(new Error(`Request "${request.header.msg_id}" timed out`))
          }
        }, this.timeout * 1000)
      }
    })
  ...
  _response (response) {
    ...
    if (this.debug) {
      console.log(requestId, responseType, response.content)
    }
