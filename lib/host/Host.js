...
class Host {
   ...
   */
  register () {
    return this.manifest().then(manifest => {
      let dir = path.join(Host.userDir(), 'hosts')
      mkdirp.sync(dir)
      fs.writeFileSync(path.join(dir, 'node.json'), JSON.stringify(manifest, null, '  '))
    })
  }
