...
class SqliteContext extends Context {
   ...
   */
  async resolve (what) {
    const rows = this._db.prepare(`SELECT count(*) FROM outputs.sqlite_master WHERE type=="table" AND name=="${what.name}"`).pluck().get()
    return rows === 1
  }
  ...
  async _unpack (input) {
    ...
    if (!value) return {name} // Input may have not been provided. Deal with better?
    if (value.type === 'table') {
