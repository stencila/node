...
class SqliteContext extends Context {
   ...
   * @override
   */
  async compile (node, type = 'cell', expr = false) {
    try {
      ...
      if (!node.type) node.type = 'cell'
      if (!node.source) node.source = {type: 'text', data: ''}
      if (!node.expr) node.expr = expr
      ...
      if (node.source.lang) {
        assert.ok(node.source.lang.match(`^sql|sqlite$`), 'Cell `source.lang` property must be either "sql" or "sqlite"')
      }
      ...
      if (outputName) node.outputs.push({name: outputName})
   ...
   * @override
   */
  async execute (node, type = 'cell', expr = false) {
    try {
      node = await this.compile(node, type, expr)
      ...
      let name
      if (node.outputs[0]) name = node.outputs[0].name
      let value = await this._pack(name, valueSql, node)
      node.outputs = [{
        name, value
      }]
