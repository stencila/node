...
class HostHttpServer {
  ...
  handle (request, response) {
    ...
    if (this._host.key !== false && auth) {
      const parts = auth.split(' ')
      if (parts[0] === 'Bearer') {
        let jwt = parts[1]
        try {
          if (!jwt) throw new Error()
          token = jsonwebtoken.verify(jwt, this._host.key, {
            algorithms: ['HS256']
          })
        } catch (err) {
  ...
  route (verb, path, token) {
    ...
    if (this._host.key !== false && !token) return [this.error403, `A valid access token is required for ${verb} ${path}`]
