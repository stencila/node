...
class Host {
      ...
      fs.access(dir, fs.constants.R_OK, error => {
        ...
        glob(path.join(dir, '*.json'), (error, files) => {
          ...
          for (let file of files) {
            ...
            if (manifest.servers) {
              try {
                key = fs.readFileSync(path.join(dir, manifest.id + '.key'), { encoding: 'utf8' })
              } catch (error) {
                // Skip this peer
                continue
              }
            ...
            if (insert) {
              ...
              this._peerKeys[manifest.id] = key
            }
          }
        })
      })
  ...
  token (peer, discover = true) {
    return Promise.resolve().then(() => {
      if (!peer) return this._key
      else {
        let key = this._peerKeys[peer]
        if (!key) {
          // No key found for this peer so discover peers and keys
          return this.discover().then(() => {
            key = this._peerKeys[peer]
            if (!key) throw new Error('Could not find a a key for peer "' + peer + '"')
            else return key
          })
        } else {
          return key
