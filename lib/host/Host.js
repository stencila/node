...
class Host {
    ...
    return Promise.resolve().then(() => {
      ...
      if (Class) {
        ...
        return Promise.resolve().then(() => {
          ...
          return this.requestPeer(peer, 'POST', '/' + type, options).then(id => {
            ...
            let instance = {proxy: true, peer, id}
            let name = makeName(type)
            this._instances[name] = instance
            return {name, instance}
          })
  ...
  discoverPeers () {
    let discoverDir = async dir => {
      fs.access(dir, fs.constants.R_OK, error => {
        if (error) return
        glob(path.join(dir, '*.json')).then(async (files) => {
          for (let file of files) {
            ...
            if (manifest.servers && manifest.servers.http) {
              try {
                manifest = await this.request(manifest.servers.http.url + '/manifest')
              } catch (error) {
                // Skip this peer
                continue
              }
  ...
  async requestPeer (peer, method, path = '', data = null) {
    let url
    let headers = {
  ...
  async generateToken (host) {
    ...
    // and so token s an empty string
    if (!peer.key) return ''
