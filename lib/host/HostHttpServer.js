...
class HostHttpServer {
  constructor (host, address = '127.0.0.1', port = 2000, key = null) {
    if (key === null) {
  ...
  /**
   * Get the key of this server
   *
   * @return {string} - Server's authentication key
   */
  get key () {
    return this._key
  }
  ...
  start () {
    return new Promise((resolve, reject) => {
      if (!this._server) {
        ...
        console.log(`Host HTTP server has started:\n  URL: ${this.url}\n  Key: ${this.key}`) // eslint-disable-line no-console
        if (this._key === false) {
  ...
  stop () {
    return new Promise((resolve) => {
      if (this._server) {
        ...
        console.log('Host HTTP server has stopped.') // eslint-disable-line no-console
      }
      resolve()
    })
  }
  ...
  handle (request, response) {
    ...
    if (this._key && auth) {
      ...
      if (parts[0] === 'Bearer') {
        ...
        try {
          ...
          token = jsonwebtoken.verify(jwt, this._key, {
            algorithms: ['HS256']
          })
        } catch (err) {
          this.error400(request, response, 'Malformed bearer token Authorization header')
        }
      }
    }
    ...
    let endpoint = this.route(request.method, uri.pathname, token)
    if (endpoint) {
      return new Promise((resolve, reject) => {
  ...
  route (verb, path, token) {
    ...
    if (path === '/environs') return [this.environs]

    // Private endpoints for which a valid token is necessary

    if (this._key && !token) return [this.error403, `A valid access token is required for ${verb} ${path}`]
