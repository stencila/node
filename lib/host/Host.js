...
class Host {
          ...
          server.start().then(() => {
            ...
            mkdirp(path.dirname(file), error => {
              ...
              this.manifest().then(manifest => {
                fs.writeFile(file, JSON.stringify(manifest, null, '  '), { mode: '600' }, error => {
                  if (error) reject(error)
                })
              })
            })
  ...
  /**
   * Create an authentication token for this host,
   * or one of it's peers
   *
   * @return {[type]} [description]
   */
  token (peer, discover = true) {
    return Promise.resolve().then(() => {
      let key
      if (!peer) key = this.key
      else {
        for (let host of this._peers) {
          if (host.id === peer) {
            key = host.key
          }
        }
        if (!key && discover) {
          // No peer host found with matching id so
          // do discovery of peers and try again
          return this.discover().then(() => {
            return this.token(peer, false)
          })
        }
        if (!key) throw new Error('Could not find a peer host with id "' + peer + '"')
      }
