...
class SqliteContext extends Context {
  constructor (...args) {
    ...
    this._db.exec('ATTACH DATABASE ":memory:" AS variables')
  }

  async resolve (what) {
    const rows = this._db.prepare(`SELECT count(*) FROM variables.sqlite_master WHERE type=="table" AND name=="${what.name}"`).pluck().get()
    return rows === 1
  }

  /**
   * An override to provide the caller with a value
   ...
   */
  async provide (what) {
    let sql = `SELECT * FROM variables.${what.name}`
  ...
  async variables () {
    return this._db.prepare('SELECT name FROM variables.sqlite_master WHERE type=="table" AND name NOT LIKE "tmp%"').pluck().all()
  }
  ...
  async _pack (name, select, node) {
    ...
    this._db.exec(`DROP TABLE IF EXISTS variables.${name}`)
    this._db.exec(`CREATE TABLE variables.${name} AS ${select}`)

    const MAX_ROWS = 10
    let pkg = await this.provide({name, limit: MAX_ROWS})

    // Decide to create a package or a data pointer
    let rowNum = this._db.prepare(`SELECT count(*) FROM variables.${name}`).pluck().get()
    if (rowNum <= MAX_ROWS) {
      return pkg
    } else {
