...
class JupyterContext extends Context {
  ...
  executeRun (run) {
    ...
    return this._request('execute_request', content, ['execute_result', 'error']).then(({request, response}) => {
      if (response.header.msg_type === 'execute_result') {
   ...
   */
  _request (requestType, content, responseTypes) {
    return new Promise((resolve, reject) => {
