...
class HostHttpServer {
  ...
  start () {
    return new Promise((resolve, reject) => {
      if (!this._server) {
        ...
        let key = this._host && this._host.key
  ...
  handle (request, response) {
    ...
    if (this._host && this._host.key && auth) {
  ...
  route (verb, path, token) {
    ...
    if (this._host && this._host.key && !token) return [this.error403, `A valid access token is required for ${verb} ${path}`]
