...
// to clients of this hos. See `Host.types`
const TYPES = {
  'Context': Context,
...
class Host {
  ...
  /**
   * Get the current execution environment of this host
   *
   * @return {Object} The current execution environment
   */
  get environ () {
    let namespace = null
    let name = null
   ...
   */
  register () {
    const dirs = [Host.userDir(), 'environs']
    const environ = this.environ
    if (environ.namespace) dirs.push(environ.namespace)
    if (environ.name) dirs.push(environ.name)
    if (environ.version) dirs.push(environ.version)
    const dir = path.join(...dirs)
    mkdirp.sync(dir)
    const json = JSON.stringify(this.manifest, null, '  ')
      ...
      if (!this._servers.http) {
        ...
        return server.start().then(() => {
          ...
          mkdirp(hostsDir, error => {
            ...
            fs.writeFile(path.join(hostsDir, this.id + '.json'), JSON.stringify(this.manifest, null, '  '), { mode: '600' }, error => {
              if (error) throw error
            })
  ...
  discoverPeers () {
    ...
    return Promise.resolve().then(() => {
      ...
      // ...then inactive hosts
      return discoverDir(path.join(Host.userDir(), 'environs', 'local'))
    }).then(() => {
